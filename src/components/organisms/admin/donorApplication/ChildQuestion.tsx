...
const ChildQuestionAdmin = ({
  ...
  const handleTextInputChange = (e: any) => {
    if (isYesQuestions && parentQuestion.yesQuestions) {
      const yesQuestions: QuestionType[] = parentQuestion.yesQuestions?.map(
        (yesQuestion) => {
    ...
    if (!isYesQuestions && parentQuestion.noQuestions) {
      const noQuestions: QuestionType[] = parentQuestion.noQuestions?.map(
        (noQuestion) => {
  ...
  return (
    <div>
              ...
              {question.options?.map((option: any, index: number) => (
                ...
                >
                  {option.type === CheckBoxType.TYPE_ONE && (
                    <>
                      {option.title && (
                        <div className="text-black text-2xl font-medium mt-5 mb-2.5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2.5">
                        <div className={`flex justify-center items-start`}>
                          <Checkbox
                            defaultChecked={
                              getInputValue(option.optionTitle) !== null &&
                              getInputValue(option.optionTitle).length > 0
                            }
                            key={`${question.id}-${index}`}
                            className={`h-[22px] w-6 rounded-md checked:bg-white border-[3px] border-primary checked:border-primary`}
                            color="gray"
            ...
            <div className="flex justify-between w-full">
              ...
              <div className="w-[275px] relative">
                <DatePicker
                  ...
                  minDate={!question?.isDateRangePast ? currentDate : null}
                  maxDate={question?.isDateRangePast ? currentDate : null}
                  disabled
                  // </DatePicker>
                />
                <div
      ...
      <div>{question.inputType === InputType.SELECT && <div></div>}</div>
    </div>
  );
};

export default ChildQuestionAdmin;
