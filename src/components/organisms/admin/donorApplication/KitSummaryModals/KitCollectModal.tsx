...
const KitCollectModal = ({ handleClose }: { handleClose: () => void }) => {
  ...
  const handleSubmit = async () => {
    ...
    const { apiError, apiSuccess } = await createKitPickup({ data: payload });
    if (apiSuccess) {
      handleClose();
    } else if (apiError) {
  ...
  const dateOfBirthHandler = async (date: any) => {
    await formik.setFieldValue("pickup_date_time", date);
    closeDatePicker();
  };
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-row justify-between gap-4">
        <div className="flex w-full flex-1 flex-col">
          ...
          <DatePicker
            ref={datePickerRef}
            selected={values.pickup_date_time}
...
export default KitCollectModal;
