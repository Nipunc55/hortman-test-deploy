...
const PublicBankingPackages = ({
  ...
  const loadPackages = async () => {
    ...
    if (apiSuccess && apiSuccess.status === 200) {
      const selectedPackage = apiSuccess?.data?.data[1];
  ...
  const handleButtonClick = () => {
    if (selectedPackage) {
      ...
      setMessage("Please select a package before proceeding.");
      // alert("Please select a package before proceeding.");
    }
  };

  return (
    <div className=" bg-white rounded-lg py-2.5 mt-4 min-w-[450px] min-h-[600px] mb-20 flex flex-col justify-between">
      ...
      <div className="mt-2.5 min-w-[450px] min-h-[500px]">
        {isLoading ? (
          <div className="flex justify-center items-center my-10 ">
            <LoaderIconSvg />
          </div>
        ) : (
          <div className="flex overflow-hidden  justify-center text-center items-center py-5">
            <div className="flex flex-row gap-8 pb-4 overflow-scroll choose-package-scrollbar">
              {packages.map((item, index) => {
                return (
                  <div
                    className={`flex flex-col items-center ${item._id === selectedPackage ? "selected-public-package" : ""}`}
                    key={index}
                  >
                    <img
                      src={item.image}
                    ...
                    <div className="flex text-center">
                      <span
                        className={`flex text-[14px] font-normal leading-5 max-w-[325px] ${
...
export default PublicBankingPackages;
