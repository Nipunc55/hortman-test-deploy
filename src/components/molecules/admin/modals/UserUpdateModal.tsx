...
import { userCreationSchema } from "../../../../utils/validations";
...
const UserUpdateModal = ({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, e) => {
      ...
      const { apiSuccess, apiError }: any = await updateUserByID(
        values.id,
        values.fullName,
        values.email,
        values.status
  ...
  const onRoleHandler = async (event: { value: string; label: string }) => {
    await formik.setFieldValue("role", event.value);
  };

  const onStatusHandler = async (event: { value: string; label: string }) => {
    await formik.setFieldValue("status", event.value);
  };
                    ...
                    .map(({ name, flags, countryCallingCode }: any) => {
                      return (
                        <MenuItem
                          ...
                          // onClick={() => setCountry(countryCallingCode)}
                        >
                          <img
                            src={flags.svg}
                            alt={name}
